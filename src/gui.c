...
Window *createWindow(char *title)
{
  ...
  gtk_window_set_default_size(GTK_WINDOW(win->gtkWindow), DEFAULT_WIDTH,
                              DEFAULT_HEIGHT);
  gtk_window_set_title(GTK_WINDOW(win->gtkWindow), title);
  g_signal_connect(G_OBJECT(win->gtkWindow), "destroy",
  ...
  gtk_box_pack_start(GTK_BOX(pane), GTK_WIDGET(win->buttonBox), FALSE,
                     TRUE, 0);
  gtk_button_box_set_layout(GTK_BUTTON_BOX(win->buttonBox),
                            GTK_BUTTONBOX_START);
               ...
               void *data)
{
  ...
  g_signal_connect_data(
    ...
    G_CALLBACK(buttonClicked), (gpointer)callbackDetails,
    (GClosureNotify)freeCallback, 0);
}
